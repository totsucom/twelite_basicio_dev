...
{
    uint8_t data[32];
    I2C_commandRead(BME280_ADDRESS, 0x88, data, 24);
    I2C_commandRead(BME280_ADDRESS, 0xA1, &data[24], 1);
    I2C_commandRead(BME280_ADDRESS, 0xE1, &data[25], 7);
...
void BME280_init() {
    ...
    I2C_writeByte(BME280_ADDRESS, 0xF2, ctrl_hum_reg);
    I2C_writeByte(BME280_ADDRESS, 0xF4, ctrl_meas_reg);
    I2C_writeByte(BME280_ADDRESS, 0xF5, config_reg);
    readTrim();
}
...
void BME280_readData(signed long int *pTemp, unsigned long int *pPres, unsigned long int *pHumi)
{
    ...
    I2C_write(BME280_ADDRESS, 0xF7, NULL, 0);
    for (i=0; i<8; i++) {
        data[i] = I2C_readByte(BME280_ADDRESS);
    }
